...
def processar_triagem(
    ...
    sintomas_texto="", comorbidades_texto="", veredito_texto="", protocolo_texto=""
):
    conn = obter_conexao()
    if not conn:
    ...
    try:
        ...
        cursor.execute(
            """
            INSERT INTO triagens (id_paciente, id_enfermeiro, consciente, pontuacao_sintomas, pontuacao_risco, pontuacao_total, classificacao, sintomas_texto, comorbidades_texto, veredito_texto, protocolo_texto)
            VALUES (%s, %s, %s, 0, 0, 0, %s, %s, %s, %s, %s) RETURNING id_triagem;
            """,
            (id_paciente, id_enfermeiro, consciente, classificacao, sintomas_texto, comorbidades_texto, veredito_texto, protocolo_texto),
        )
